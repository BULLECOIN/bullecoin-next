...
function ata(owner:PublicKey,mint:PublicKey){return PublicKey.findProgramAddressSync([owner.toBuffer(),TOKEN_PROGRAM.toBuffer(),mint.toBuffer()],ATA_PROGRAM)[0]}
function createAta(payer:PublicKey,address:PublicKey,owner:PublicKey,mint:PublicKey){return new TransactionInstruction({programId:ATA_PROGRAM,keys:[{pubkey:payer,isSigner:true,isWritable:true},{pubkey:address,isSigner:false,isWritable:true},{pubkey:owner,isSigner:false,isWritable:false},{pubkey:mint,isSigner:false,isWritable:false},{pubkey:SystemProgram.programId,isSigner:false,isWritable:false},{pubkey:TOKEN_PROGRAM,isSigner:false,isWritable:false}],data:Buffer.alloc(0)})}
function transferChecked(source:PublicKey,mint:PublicKey,destination:PublicKey,owner:PublicKey,amount:bigint,decimals:number){const data=Buffer.alloc(10);data.writeUInt8(12,0);data.writeBigUInt64LE(amount,1);data.writeUInt8(decimals,9);return new TransactionInstruction({programId:TOKEN_PROGRAM,keys:[{pubkey:source,isSigner:false,isWritable:true},{pubkey:mint,isSigner:false,isWritable:false},{pubkey:destination,isSigner:false,isWritable:true},{pubkey:owner,isSigner:true,isWritable:false}],data})}
...
export async function sendBulleReward(recipientAddress:string,uiAmount:number){
 ...
 const connection=new Connection(rpc,"confirmed"),payer=readPayer(),recipient=new PublicKey(recipientAddress),mint=new PublicKey(BULLE_MINT),source=ata(payer.publicKey,mint),destination=ata(recipient,mint),mintInfo=await connection.getAccountInfo(mint,"confirmed");
 if(!mintInfo||!mintInfo.owner.equals(TOKEN_PROGRAM)||mintInfo.data.length<45)throw new Error("Unsupported or invalid BULLE mint");
 ...
 const transaction=new Transaction();if(!await connection.getAccountInfo(destination,"confirmed"))transaction.add(createAta(payer.publicKey,destination,recipient,mint));transaction.add(transferChecked(source,mint,destination,payer.publicKey,rawAmount,decimals));
