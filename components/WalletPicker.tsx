...
export default function WalletPicker({ onClose, onConnected }: { onClose: () => void; onConnected: (provider: BrowserWalletProvider, address: string) => void | Promise<void> }) {
  ...
  return createPortal(<div className="bulleWalletModal" role="dialog" aria-modal="true" aria-label="Connect a Solana wallet"><section><header><div><small>BULL HUB // JUPITER WALLET ACCESS</small><h2>CONNECT WALLET</h2><p>{mobile?"Open BulleCoin inside your wallet browser, then use the same wallet to approve Jupiter swaps.":"Choose an installed Solana wallet. Jupiter opens inside Bull Hub and every swap still requires your approval."}</p><span className="bulleWalletDetected">{installedCount?`${installedCount} WALLET${installedCount>1?"S":""} DETECTED`:mobile?"CHOOSE AN APP":"NO EXTENSIONS DETECTED"}</span></div><button onClick={onClose}>×</button></header><div className="bulleWalletChoices">{wallets.map(wallet=>{const installed=Boolean(getProvider(wallet.name));const action=installed?"CONNECT":mobile?"OPEN APP ↗":"GET ↗";return <button key={wallet.name} onClick={()=>void choose(wallet.name)}><i style={{"--wallet-tone":wallet.tone} as React.CSSProperties}>{wallet.mark}</i><span><b>{wallet.name}</b><small>{installed?"READY FOR BULL HUB":mobile?"OPEN IN-APP BROWSER":"INSTALL EXTENSION"}</small></span><strong>{action}</strong></button>})}</div>{status&&<p className="bulleWalletStatus">{status}</p>}<footer>Jupiter-compatible access. Bull Hub never requests a seed phrase or private key.</footer></section></div>,document.body);
}
